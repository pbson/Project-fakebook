...
router.post("/get_list_suggested_friends", async (req, res) => {
  const { token, index, count } = req.query;
  try {
    //Decode token to get user_id
    jwt.verify(token, "secretToken", async (err, userData) => {
      if (err) {
        ...
        let requestData = user.FriendsRequest.map(async friend => {
          let findFriend = await User.findOne({ _id: friend });
          return {
            id: findFriend.id,
            username: findFriend.username,
        ...
        let filteredArray = allUser.filter(a => !requestData.some(b => a.id === b.id));
